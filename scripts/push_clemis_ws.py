...
incident_tree = connectdispatch.clemis.getxml(clemis_cfg['ws_hrs'], clemis_cfg['ws_url'], clemis_cfg['cad_user'],
                                              clemis_cfg['cad_pass'])

# Get incident list (agency_code & incident_number)
incident_list = connectdispatch.clemis.incidentlist(incident_tree)
...
incident_list_new = connectdispatch.clemis.comparelists(incident_list, incident_list_check)
    ...
    for i in incident_list_full:
        for new in incident_list_new:
            if i['incident_number'] == new['incident_number']:
                i = connectdispatch.clemis.incidentdict_ws(i, unit_list_full, push=True)
